...
<body <?php body_class('family-tree-page'); ?>>
    <nav class="top-menu"><a href="/family-dashboard">🏠 Dashboard</a><a href="/browse-members"
            class="active">Members</a><a href="/browse-clans">Clans</a></nav>
        ...
        <form id="editMemberForm">
            ...
            <div class="form-row">
                ...
                <select name="clan_id" id="clan_id">
                    ...
                    <?php foreach ($clans as $c): ?>
                        <option value="<?php echo intval($c->id); ?>" <?php selected($member->clan_id, $c->id); ?>>
                            <?php echo esc_html($c->clan_name); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-row">
                ...
                <select name="parent1_id">
                    ...
                    <?php foreach ($all_members as $m):
                        if ($m->id == $member->id)
                            continue; ?>
                        <option value="<?php echo intval($m->id); ?>" <?php selected($member->parent1_id, $m->id); ?>>
                            <?php echo esc_html($m->first_name . ' ' . $m->last_name); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-row">
                ...
                <select name="parent2_id" required>
                    ...
                    <?php foreach ($all_members as $m):
                        if ($m->id == $member->id)
                            continue; ?>
                        <option value="<?php echo intval($m->id); ?>" <?php selected($member->parent2_id, $m->id); ?>>
                            <?php echo esc_html($m->first_name . ' ' . $m->last_name); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-row">
                ...
                <label>Country</label><input type="text" name="country"
                    value="<?php echo esc_attr($member->country); ?>">
    ...
    <script>
        jQuery(function ($) {
            ...
            $('#editMemberForm').on('submit', function (e) {
                e.preventDefault();
                // Both parents optional now - removed validation
                // At least one parent should exist (optional check)
                // if (!$('select[name="parent1_id"]').val() && !$('select[name="parent2_id"]').val()) {
                //     console.log('Creating root ancestor with no parents');
                // }
